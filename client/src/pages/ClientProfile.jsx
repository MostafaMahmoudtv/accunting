...
const ClientProfile = () => {
  ...
  const { data: activity } = useQuery({
    ...
    queryFn: async () =>
      (await api.get(`/activity?entityType=Client&entityId=${id}&limit=30`))
        .data.data,
  });
  ...
  const addNoteMut = useMutation({
    mutationFn: async (body) =>
      (await api.post(`/clients/${id}/notes`, body)).data.data,
    onSuccess: () => {
  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      <div className="flex items-center gap-2 sm:gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-lg sm:text-2xl font-bold tracking-tight flex flex-wrap items-center gap-1.5 sm:gap-2">
            ...
            <StatusBadge
              status={
                client.clientType === "one_time" ? "pending" : "in_progress"
              }
              label={t(
                `clients.${client.clientType === "one_time" ? "oneTime" : client.clientType}`,
              )}
            />
            <StatusBadge
              status={client.status}
              label={t(`clients.${client.status}`)}
            />
            <StatusBadge
              status={client.paymentStatus}
              label={t(
                `common.${client.paymentStatus === "partially_paid" ? "partial" : client.paymentStatus}`,
              )}
            />
          </h1>
      ...
      <div className="card p-3 sm:p-5 grid grid-cols-1 xs:grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
        ...
        <Field
          icon={Calendar}
          label={t("clients.startDate")}
          value={fmtDate(client.startDate, i18n.language)}
        />
        {client.endDate && (
          <Field
            icon={Calendar}
            label={t("clients.endDate")}
            value={fmtDate(client.endDate, i18n.language)}
          />
        )}
        {client.monthlyFee > 0 && (
          <Field
            ...
            value={fmtMoney(client.monthlyFee, "SAR", i18n.language)}
          />
        )}
        <Field
          icon={UserIcon}
          label={t("clients.assignedAccountant")}
          value={client.assignedAccountant?.name}
        />
        <Field
          icon={UserIcon}
          label={t("clients.assignedCustomerService")}
          value={client.assignedCustomerService?.name}
        />
      </div>
                ...
                >
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-ink-500 truncate">
                      {fmtDate(task.dueDate, i18n.language)} •{" "}
                  ...
                  <div key={p._id} className="p-3 space-y-1">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <StatusBadge
                        status={p.status}
                        label={t(
                          `common.${p.status === "partially_paid" ? "partial" : p.status}`,
                        )}
                      />
                    </div>
                    <div className="flex items-center justify-between gap-2 text-xs">
                      <span className="text-ink-500">
                        {fmtDate(p.paymentDate, i18n.language)}
                      </span>
                      <span className="font-semibold">
                        {fmtMoney(p.amount, "SAR", i18n.language)}
                      </span>
                    </div>
                  </div>
                ))}
        ...
        <div className="card p-4 sm:p-5 space-y-4">
          ...
          {documents?.length ? (
                ...
                <li key={doc._id} className="flex items-center gap-3 py-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-ink-500">
                      {doc.uploadedBy?.name || "—"} • {fmtSize(doc.size)} •{" "}
                      {fmtRelative(doc.createdAt, i18n.language)}
                    </div>
                  </div>
                  <a
                    href={
            ...
            <EmptyState
              ...
              message="Documents uploaded for this client will appear here"
              icon={FileText}
            />
          )}
        </div>
              ...
              {client.timeline.map((n) => (
                ...
                >
                  ...
                  <div className="text-[11px] text-ink-500 mt-1.5">
                    {n.createdBy?.name || "—"} •{" "}
                    {fmtRelative(n.createdAt, i18n.language)}
                  </div>
                </div>
              ))}
            </div>
                ...
                <li key={a._id} className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-[11px] text-ink-500 mt-0.5">
                      {fmtRelative(a.createdAt, i18n.language)}
                    </div>
                  </div>
                </li>
              ))}
        ...
        >
          ...
          <div className="flex items-center justify-end gap-2 mt-3">
            ...
            <button
              ...
              disabled={!noteText || addNoteMut.isPending}
            >
              {t("common.save")}
            </button>
          </div>
...
const Field = ({ icon: Icon, label, value }) => (
  <div className="flex items-start gap-2.5 min-w-0">
    ...
    <div className="min-w-0 flex-1">
      <div className="text-[10px] sm:text-[11px] uppercase text-ink-500 tracking-wide">
        {label}
      </div>
